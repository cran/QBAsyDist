#' @title Semiparametric quantile regression in generalized normal distributional settings.
#' @description The local polynomial technique is used to estimate location and scale function of the quantile-based asymmetric normal distribution discussed in Gijbels et al. (2019b) and Gijbels et al. (2019c). Using these estimates, the quantile function of the generalized asymmetric normal distribution will be estimated. A detailed study can be found in Gijbels et al. (2019b).
...
#' @param alpha This is the index parameter  \eqn{\alpha} of the generalized asymmetric normal density. The default value of \eqn{\alpha} is NULL in the code \code{\link{SemiQRegGAND}}. In this case, the \eqn{\alpha} will be estimated based on the residuals from local linear mean regression.
...
#' @return The code \code{\link{SemiQRegGAND}} provides the realized value of the \eqn{\beta}th conditional quantile estimator by using semiparametric quantile regression technique discussed in Gijbels et al. (2019b) and Gijbels et al. (2019c).
#' @import locpol
#' @rdname SemiQRegGAND
#'
#'
...
#' @rdname SemiQRegGAND
#' @export
SemiQRegGAND<-function(beta,x, y,p1=1,p2=1,  h,alpha=NULL,g,lower = -Inf, upper = Inf, m = 101){
  z=g(y)
  g.inv<-GoFKernel::inverse(g,lower,upper)
  SemiQReg_Z<-SemiQRegAND(beta,x, z,  p1,p2,  h,alpha, m)
  qf_g<-NA
  for(i in 1:length(SemiQReg_Z$fit_beta_AND)){
    qf_g[i]<-g.inv(SemiQReg_Z$fit_beta_AND[i])}
